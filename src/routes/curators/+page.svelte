<script>
	import Logo from '$lib/components/logo/logo.svelte';
</script>

<div class="flex w-screen items-center overflow-y-auto bg-zinc-900 p-4">
	<div class="flex max-w-2xl flex-col gap-8 text-zinc-100">
		<div class="w-32 fill-zinc-100">
			<Logo />
		</div>

		<h1 class="font-bold">Public Shorts: Video Festival in Berlin 2026, Curated by <i>you?</i></h1>

		<p class="text-zinc-300">
			Hi, if you’ve received this link, it’s because you’re invited to take part in the curation of
			the upcoming <i>Public Shorts</i> video festival.
		</p>

		<h2 class="font-bold">Public Shorts?</h2>
		<p class="text-zinc-300">
			We are putting together a video festival, which will stream 24/7 on screens inside the glass
			window of <a href="https://kanape.studio">kanapé</a>, which means that anyone could come and
			watch the shorts during the festival period
		</p>
		<div class=""></div>
		<p>
			On top of this, we will have 3 screening sessions with the different shorts, with Q&A,
			debates, etc.
		</p>
		<h2 class="font-bold">What does that entail?</h2>
		<p class="text-zinc-300">
			You’ll review submitted works, take notes (Phase 1), and join collective discussions (Phase 2)
			to shape the final selection. We're excpecting a lot of submissions so we are trying to gather
			a diverse and numerous group of curators so it remains manageable for everyone. We planned two
			physical meetings at Kanapé in Berlin to facilitate discussions and final decisions.
		</p>

		<h2 class="font-bold">Timeline and key dates</h2>
		<ul class="list-inside list-disc text-zinc-300">
			<li><b>Open Call:</b> 12 Dec 2025 – 10 Jan 2025</li>
			<li><b>Curation Phase 1 (Individual viewing):</b> 19 Jan – 31 Jan</li>
			<li><b>Curation Phase 2 (Collective curation):</b> 1 Feb – 15 Feb</li>
			<li><b>Physical meetings at Kanapé:</b> 6 & 13 Feb</li>
			<li><b>Preparation of space and events:</b> 16 Feb – 27 Feb</li>
			<li><b>Festival dates:</b> 27 Feb – 15 Mar</li>
			<li><b>Festival opening and first screenings:</b> 27 Feb</li>
			<li><b>Festival second screening session:</b> 27 Feb – 31 Mar</li>
			<li><b>Festival closing and last screening session:</b> 27 Feb – 15 Mar</li>
		</ul>
		<h2 class="font-bold">Is this paid?</h2>
		<p>
			No. We are all doing this together as volunteers, we are searching for fundings and sponsors
			but are not counting on it. 🤷 Maybe next year!
		</p>
		<h2 class="font-bold">Next step</h2>
		<p class="text-zinc-300">
			Please confirm your participation with the person who sent you this link. Thank you so much
			for your help :)
		</p>
	</div>
</div>
