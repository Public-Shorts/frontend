		...
		<div class="md:order-last md:col-span-2">
			...
			>
				...
				<p class=" ">
					...
					page before submitting, or contact us at
			...
			>
						...
						>
							{#each CATEGORIES as category}
								<label class="flex cursor-pointer items-center space-x-2">
									...
									<span
										class="text-sm"
										class:opacity-50={!selectedCategories.includes(category.value) &&
											selectedCategories.length >= 5}>{category.title}</span
									>
								</label>
							{/each}
						</div>
						<p class="mt-1 text-sm text-gallery-500">
							Please select maximum 5 categories that apply to your video. ({selectedCategories.length}/5
							selected)
						</p>
				...
				<section class="space-y-4">
					...
					<div>
						...
						/>
						<p class="text-sm text-gallery-500 mt-1">
							If we need a password to see your video, add it here
						</p>
					</div>
					...
					<div>
						...
						/>
						<p class="mt-1 text-sm text-gallery-500">
							Make sure the link is working and does not expire – like WeTransfer. It's best to use
							a drive or cloud service. Please provide a file with English subtitles.
						</p>
					...
					<div>
						...
						{#if screenshotFiles.length > 0}
							<div
								class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3 p-3 bg-gallery-50 border border-gallery-300 rounded"
							>
									...
									>
										...
										>
											...
											{#if screenshot.uploading}
												<div
													class="absolute inset-0 bg-gallery-900 bg-opacity-50 flex items-center justify-center"
												>
										...
										<div class="p-2 text-xs">
											<p class="truncate text-gallery-700 font-medium">
												{screenshot.file?.name || 'Uploaded image'}
											</p>
											{#if screenshot.file}
										...
										<div
											class="absolute top-1 left-1 bg-gallery-900 text-white px-2 py-0.5 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity"
										>
											{index + 1}
										</div>
									</div>
						...
						{#if screenshotFiles.length < 5}
							<label
								for="screenshots"
								class="relative block w-full p-4 bg-gallery-50 border-2 border-gallery-300 rounded cursor-pointer hover:border-accent-500 transition-colors text-center"
							>
								<span
									class="inline-block px-4 py-2 bg-accent-500 text-white rounded font-medium hover:bg-accent-600 transition-colors"
								>
									Choose Files
								</span>
								<span class="block mt-2 text-sm text-gallery-600"> or drag and drop </span>
								<input
									type="file"
									id="screenshots"
									accept="image/*"
						...
						<p class="text-sm text-gallery-500 mt-1">
							Please attach up to 5 video stills from your work. Drag to reorder. Please keep the file size below 3MB each.
						</p>
						{#if screenshotFiles.length === 0}
					...
					<div>
						...
						{#if posterFile}
							<div class="mb-3 p-3 bg-gallery-50 border border-gallery-300 rounded">
								<div
									class="relative group max-w-xs mx-auto bg-white rounded border-2 border-gallery-300"
									class:opacity-50={posterFile.uploading}
								>
									<button
										type="button"
									...
									<div class="p-2 text-xs">
										<p class="truncate text-gallery-700 font-medium">
											{posterFile.file?.name || 'Uploaded image'}
										</p>
										{#if posterFile.file}
				...
				<section class="space-y-4">
						...
						{#if aiUsed}
							<div class="ml-6">
								<label for="aiExplanation" class="mb-2 block text-sm">
									If yes, please tell us in a short text how AI was used: <span class="text-red-500"
										>*</span
									>
								</label>
								<textarea
					...
					<div class="space-y-2">
						<label class="flex items-start">
							<input
								...
								value="true"
								bind:checked={previousScreenings}
								class="mt-1 mr-2"
							/>
							<span>
					...
					<div>
						<label for="specialRequirements" class="mb-2 block"
							>Special Screening Requirements</label
						>
						<textarea
					...
					<label class="flex items-start">
						...
						<span>
							...
							<ul class="mt-1 ml-4 block list-disc space-y-1 text-sm text-gallery-500">
								...
								<li>
									I accept that the festival may use this information, including the screenshots,
									when promoting the festival, whether for non-commercial or commercial endeavors
								</li>
