<script lang="ts">
	import GridLayout from '../../lib/components/GridLayout.svelte';
</script>

<GridLayout>
	<!-- English Section -->
	<h1 class="mb-4 text-3xl font-bold md:col-span-5 md:col-start-2">Terms & Conditions</h1>

	<div class="font-semibold md:col-span-1">Agreement</div>
	<div class="md:col-span-5">
		<p>
			By submitting a work (the “Video”) to the Public Shorts Video Festival, the submitting person
			or entity (“Artist”) agrees to the following Terms and Conditions.
		</p>
	</div>

	<div class="font-semibold md:col-span-1">1. Eligibility & Ownership</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				The Artist confirms being the creator and/or authorized rights holder of the Video and all
				associated materials (including sound, images, text, and graphics) and is fully entitled to
				grant the rights described herein.
			</li>
			<li>
				The Artist guarantees that all information provided in the submission form is true,
				accurate, and complete.
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">2. Festival Presentation</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				The Artist grants the Festival a non-exclusive, royalty-free, worldwide license to exhibit
				the Video during the Festival on all Festival-controlled screens, windows, and projection
				surfaces, including continuous loops visible from public space.
			</li>
			<li>This includes rights for official events, test screenings, and internal previews.</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">3. Promotion & Usage</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				The Festival may use submitted information (title, artist name, synopsis, etc.) for
				promotional purposes in print, online, and social media.
			</li>
			<li>
				The Artist expressly agrees that the Festival may use still images, screenshots from the
				Video, and press images for promoting the Festival in both non-commercial and commercial
				contexts (e.g., press releases, posters, website, social media).
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">4. Copyright & Liability</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				The Artist confirms the Video contains no unauthorized copyright-protected material (music,
				footage, etc.) and infringes no third-party rights.
			</li>
			<li>
				The Artist agrees to indemnify the Festival against any claims or costs arising from rights
				violations or incorrect information.
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">5. Prohibited Content (Germany)</div>
	<div class="md:col-span-5">
		<p class="mb-2">
			By submitting, you confirm that the content does not violate the laws of the Federal Republic
			of Germany. The following are strictly prohibited and will result in immediate
			disqualification:
		</p>
		<ul class="list-disc space-y-2 pl-4">
			<li>
				<strong>Hate Speech & Incitement:</strong> Content that incites hatred, violence, or
				discrimination against individuals or groups based on nationality, ethnicity, religion,
				gender, sexual orientation, or disability (in accordance with § 130 StGB -
				<em>Volksverhetzung</em>).
			</li>
			<li>
				<strong>Unconstitutional Symbols:</strong> The use of symbols, insignia, or propaganda of unconstitutional
				organizations (e.g., Nazi symbols) is prohibited unless used clearly for educational, artistic,
				or documentary purposes (§ 86a StGB).
			</li>
			<li>
				<strong>Protection of Minors:</strong> Since screens are visible in public space 24/7,
				content must not be pornographic or deemed "harmful to minors" (indexed or rated 18+) under
				the German Protection of Young Persons Act (<em>JuSchG</em>).
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">6. Screening Rights</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				Selected Videos may be shown multiple times during the Festival without additional fees.
			</li>
			<li>
				The Festival may include the Video in "best-of" or recap programs directly related to the
				specific Festival edition.
			</li>
			<li>
				All other rights remain with the Artist; copyright is not transferred to the Festival.
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">7. Archiving</div>
	<div class="md:col-span-5">
		<p>
			The Festival may keep a digital copy of the Video and materials in its internal archive for
			documentation and non-commercial research purposes.
		</p>
	</div>

	<div class="font-semibold md:col-span-1">8. Selection & Data</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				Submission does not guarantee selection. The Festival reserves the right to remove a Video
				for technical or legal reasons.
			</li>
			<li>
				The Artist consents to the processing of personal data for Festival administration and
				communication.
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">9. Jurisdiction</div>
	<div class="md:col-span-5">
		<p>
			These Terms are governed by the laws of the Federal Republic of Germany. Place of jurisdiction
			is Berlin.
		</p>
	</div>

	<!-- German Section -->
	<h1
		class="border-t border-t-2 border-gallery-300 pt-4 pt-12 text-3xl font-bold md:col-span-5 md:col-start-2"
	>
		Teilnahmebedingungen
	</h1>

	<div class="font-semibold md:col-span-1">Vereinbarung</div>
	<div class="md:col-span-5">
		<p>
			Mit der Einreichung eines Werkes (das „Video“) beim Public Shorts Video Festival stimmt die
			einreichende Person oder Organisation („Künstler*in“) den folgenden Bedingungen zu.
		</p>
	</div>

	<div class="font-semibold md:col-span-1">1. Rechte & Echtheit</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				Die Künstler*in bestätigt, Urheber*in und/oder berechtigte*r Rechteinhaber*in aller
				Bestandteile des Videos zu sein.
			</li>
			<li>Alle Angaben im Einreichungsformular sind wahrheitsgemäß und vollständig.</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">2. Vorführrechte</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				Das Festival erhält das nicht-exklusive Recht, das Video während der Laufzeit auf allen
				Screens und Fenstern (auch im öffentlichen Raum) zu zeigen.
			</li>
			<li>Dies umfasst auch Testvorführungen und interne Sichtungen.</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">3. Nutzung & Promotion</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				Informationen zum Werk dürfen für werbliche und informative Zwecke (Print, Web, Social
				Media) genutzt werden.
			</li>
			<li>
				Das Festival darf Standbilder und Screenshots für die Bewerbung des Festivals in
				nicht-kommerziellen und kommerziellen Kontexten verwenden.
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">4. Urheberrechte Dritter</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>Das Video darf keine Rechte Dritter verletzen und kein unbefugtes Material enthalten.</li>
			<li>
				Die Künstler*in stellt das Festival von allen Ansprüchen frei, die aus Rechtsverletzungen
				resultieren.
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">5. Verbotene Inhalte (Deutschland)</div>
	<div class="md:col-span-5">
		<p class="mb-2">
			Mit der Einreichung bestätigen Sie, dass der Inhalt nicht gegen die Gesetze der Bundesrepublik
			Deutschland verstößt. Folgende Inhalte sind streng untersagt und führen zum Ausschluss:
		</p>
		<ul class="list-disc space-y-2 pl-4">
			<li>
				<strong>Hassrede & Volksverhetzung:</strong> Inhalte, die zu Hass, Gewalt oder Diskriminierung
				aufstacheln (gemäß § 130 StGB).
			</li>
			<li>
				<strong>Verfassungswidrige Kennzeichen:</strong> Symbole verfassungswidriger Organisationen (z.
				B. NS-Symbolik) sind verboten, außer sie dienen eindeutig der Kunst oder Aufklärung (§ 86a StGB).
			</li>
			<li>
				<strong>Jugendschutz:</strong> Da die Bildschirme öffentlich einsehbar sind, dürfen Inhalte
				nicht pornografisch oder jugendgefährdend sein (Jugendschutzgesetz – <em>JuSchG</em>).
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">6. Archivierung & Datenschutz</div>
	<div class="md:col-span-5">
		<ul class="list-disc space-y-2 pl-4">
			<li>
				Das Festival darf eine Kopie des Videos zu Archiv- und Dokumentationszwecken behalten.
			</li>
			<li>
				Personenbezogene Daten werden zur Durchführung des Festivals gespeichert und verarbeitet.
			</li>
		</ul>
	</div>

	<div class="font-semibold md:col-span-1">7. Gerichtsstand</div>
	<div class="md:col-span-5">
		<p>Es gilt das Recht der Bundesrepublik Deutschland. Gerichtsstand ist Berlin.</p>
	</div>
</GridLayout>
